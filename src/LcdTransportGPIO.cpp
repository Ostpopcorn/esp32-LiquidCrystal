...
#include "LcdTransportGPIO.h"
#include "delayMicroseconds.h"


LcdTransportGPIO::LcdTransportGPIO(bit_mode pinmode,gpio_num_t rs, gpio_num_t rw, gpio_num_t enable,
...
void LcdTransportGPIO::write8bits(uint8_t value) {
  for (int i = 0; i < 8; i++) {
    gpio_set_level(_data_pins[i], (value >> i) & 0x01);
  }
...
void LcdTransportGPIO::pulseEnable(void) {
...
void LcdTransportGPIO::setDataPin(uint8_t pin, gpio_num_t gpio){
  _data_pins[pin] = gpio;
}
void LcdTransportGPIO::setCtrlPin(ctrl_pins pin, gpio_num_t gpio){
